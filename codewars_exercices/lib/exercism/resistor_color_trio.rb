...
find original: https://exercism.org/tracks/ruby/exercises/resistor-color-trio/edit

=end

class ResistorColorTrio
  HASH = {black: 0, brown: 1, red: 2, orange: 3, yellow: 4, green: 5,
  ...
  def label
    arr = @color.map(&:to_sym)
    s  = "#{HASH[arr[0]]}#{HASH[arr[1]]}"
    s << "0" * HASH[arr[2]]
    s.chars.last(3).join == "000" ? s.delete_suffix!("000") << " kiloohms" : s << " ohms"
    "Resistor value: " << s
  end
end
