...
find original: https://app.codesignal.com/arcade/code-arcade/lab-of-transformations/bBgu7NEfk2GoQuNzf/solutions

=end

def alpha_less(s1, s2)
   sub = ->(x) { x.rjust(20,?0) }
    a = s1.gsub(/[[:digit:]]+/, &sub)
    b = s2.gsub(/[[:digit:]]+/, &sub)
    return true if a < b
    return false if a > b
    s1.ljust(20,?Z) < s2.ljust(20,?Z)
